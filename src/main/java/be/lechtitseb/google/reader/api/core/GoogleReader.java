...
public final class GoogleReader implements
	...
	private static final Logger LOG = Logger.getLogger(GoogleReader.class
			.getName());
	 ...
	 *             If the user is not authenticated
	 */
	public List<Item> search(String searchTerm, Integer numberOfElements)
			throws GoogleReaderException {
	 ...
	 *             If the user is not authenticated
	 */
	public List<Item> search(String searchTerm, FeedDescriptor feedDescriptor)
			throws GoogleReaderException {
	...
	public List<Item> search(String searchTerm, FeedDescriptor feedDescriptor,
		...
		String searchResults = api.search(searchTerm, feedDescriptor,
				numberOfElements, OutputFormat.JSON);
		List<String> resultsIds = GoogleReaderUtil
				.getItemIdsFromJson(searchResults);
	...
	public String getNextUnreadItem() throws GoogleReaderException {
		return api.getNextUnreadItem(this.getUserPreferences()
				.getShuffleToken());
	}
	 ...
	 *             If the user is not authenticated
	 */
	public String getNextUnreadItem(String shuffleToken)
			throws GoogleReaderException {
	...
	/**
	 * @author maratische Mark the item from a feed as read
	 * @param feed
	 *            The feed to mark as read
	 * @throws GoogleReaderException
	 *             If the user is not authenticated
	 */
	public void markItemAsRead(String itemUrl, String feedId)
			throws GoogleReaderException {
		api.markItemAsRead(itemUrl, feedId);
	}

	/**
	 * @author maratische Mark the item from a feed as Starred
	 * @param feed
	 *            The feed to mark as starred
	 * @throws GoogleReaderException
	 *             If the user is not authenticated
	 */
	public void markItemAsStarred(String itemUrl, String feedId)
			throws GoogleReaderException {
	 ...
	 *             If the user is not authenticated
	 */
	public void markItemAsUnStarred(String itemUrl, String feedId)
			throws GoogleReaderException {
	 ...
	 * easily.
	 * <p>
	 * continuation和pastNDay只有一个可以用到，当continuation为空的时候，说明是第一次获取，使用pastNday参数；
	 * 当continuation不为空时，说明是连续获取，不再使用pastNDay参数。
	 * 
	 * @param feedUrl
	...
	public String getFeedItems(String feedUrl, Integer numberOfElements,
		...
		return api.getFeedItems(feedUrl, numberOfElements, continuation,
				pastNDay);
	}

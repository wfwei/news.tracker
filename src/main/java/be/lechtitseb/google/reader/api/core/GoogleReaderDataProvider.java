 ...
 */
public final class GoogleReaderDataProvider implements
		AuthenticationManager<GoogleCredentials> {
	private static final Logger LOG = Logger
			.getLogger(GoogleReaderDataProvider.class.getName());
	...
	protected GoogleReaderDataProvider(ProxyCredentials proxyCredentials) {
		...
		/*
		 * if (proxyCredentials == null) { } else { httpManager = new
		 * ProxyHttpManager(proxyCredentials); }
		 */
	}
	 ...
	 */
	public String getFeedItems(String feedUrl, Integer numberOfElements,
			String continuation, Integer pastNDay) throws GoogleReaderException {
		LOG.trace("Getting feed items for feed: " + feedUrl);
		...
		if (continuation != null) {
			parameters.add(new Parameter("c", continuation));
		} else if (pastNDay != null) {
			parameters.add(new Parameter("ot", System.currentTimeMillis()
					/ 1000 - pastNDay * 24 * 60 * 60));
		} else {
			throw new IllegalArgumentException(
					"The continuation and pastNDay cannot be both null!");
		}
		// 从老到新的顺序
		parameters.add(new Parameter("r", "o"));
	...
	public String getItem(String itemId) throws GoogleReaderException {
		...
		return httpManager
				.post(Constants.URL_SEARCH_CONTENTS, parameters, true);
	}
	 ...
	 *             If the user is not authenticated
	 */
	public String getReadItems(FeedDescriptor feedDescriptor)
			throws GoogleReaderException {
	...
	public String getReadItems(String feedId, Integer numberOfElements)
		...
		parameters.add(new Parameter(Constants.PARAMETER_STATE_FILTER,
				Constants.FILTER_CURRENT_USER_UNREAD));
		return httpManager.get(url, parameters, true);
	}
	...
	public String getSharedFriendsItems(Integer numberOfElements)
		...
		parameters.add(new Parameter(Constants.PARAMETER_STATE_FILTER, "user/"
				+ getUserId() + Constants.ITEM_STATE_READ));
	...
	public String getSharedItems(String userId, Integer numberOfElements)
		...
		String url = Constants.URL_ITEMS_SHARED_BY_SOMEONE_ELSE.replace(
				Constants.URL_USER_TOKEN, urlEncode("" + userId));
	 ...
	 *             If the user is not authenticated
	 */
	public String getSubscriptions(OutputFormat outputFormat)
			throws GoogleReaderException {
	...
	public String getUnreadItems(String feedId, Integer numberOfElements)
		...
		parameters.add(new Parameter(Constants.PARAMETER_STATE_FILTER, "user/"
				+ userId + Constants.ITEM_STATE_READ));
		parameters.add(new Parameter("ck", "" + new Date().getTime()));
		parameters.add(new Parameter("client", "greader-unofficial"));
		GregorianCalendar calendar = new GregorianCalendar();
		calendar.add(GregorianCalendar.MONTH, -1);
		parameters.add(new Parameter("ot", ""
	...
	public boolean login() throws AuthenticationException {
		...
		List<Parameter> parameters = new ArrayList<Parameter>();
		parameters.add(new Parameter(Constants.PARAMETER_LOGIN_USERNAME,
				credentials.getUsername()));
		parameters.add(new Parameter(Constants.PARAMETER_LOGIN_PASSWORD,
		...
		parameters.add(new Parameter("source",
				"greader-unofficial-0.1-SNAPSHOT"));
		...
		try {
			...
			// Parse the result
			String sid = result.substring(result.indexOf(Constants.SID_TAG)
					+ Constants.SID_TAG.length(), result.indexOf('\n'));
			// LOG.debug("SID: "+sid);
			result = result.substring(result.indexOf('\n') + 1);
			String lSid = result.substring(result.indexOf(Constants.LSID_TAG)
					+ Constants.LSID_TAG.length(), result.indexOf('\n'));
			result = result.substring(result.indexOf('\n') + 1);
			String auth = result.substring(result.indexOf(Constants.AUTH_TAG)
			...
			throw new AuthenticationException(
					"Authentication failed using the provided credentials (they may be wrong or there was connectivity problem)",
					e);
		}
	}
	...
	public String search(String searchTerm, FeedDescriptor feed,
		...
		parameters.add(new Parameter(Constants.PARAMETER_SEARCH_TERM,
				urlEncode(searchTerm)));

		if (feed != null) {
			// The feed id needs to be encoded
			parameters.add(new Parameter(
					Constants.PARAMETER_SEARCH_SOURCE_FEED, urlEncode(feed
							.getId())));
		}
		if (numberOfElements != null) {
			parameters.add(new Parameter(
	 ...
	 *             If the user is not authenticated
	 */
	public String getReadingListByLabel(String label)
			throws GoogleReaderException {
	 ...
	 *             If the user is not authenticated
	 */
	public String getReadingListByLabel(String label, Integer numberOfElements)
			throws GoogleReaderException {
	...
	public String getUserInformation() throws GoogleReaderException {
		...
		List<Parameter> parameters = new ArrayList<Parameter>();
		parameters.add(new Parameter("Authorization", "GoogleLogin auth="
				+ credentials.getAuth()));
		// parameters.add(new
	 ...
	 *             If the user is not authenticated
	 */
	public void markFeedAsRead(FeedDescriptor feed)
			throws GoogleReaderException {
		if (feed == null) {
	...
	public void markFeedAsRead(String feedId) throws GoogleReaderException {
		...
		parameters.add(new Parameter(Constants.PARAMETER_TOKEN, getToken()));
		String result = httpManager.post(Constants.URL_MARK_ALL_AS_READ,
				parameters, true);

		if (!"OK".equals(result)) {
			throw new GoogleReaderException(
					"The operation failed (no more details, sorry)");
		}
	}
	 ...
	 *             If the user is not authenticated
	 */
	public void markItemAsRead(ItemDescriptor item, FeedDescriptor feed)
			throws GoogleReaderException {
		if (item == null) {
	...
	/**
	 * Mark the item from a feed as Starred this method is dublicaed from
	 * markItemAsRead - TODO I should think about it, optimize
	 * 
	 * @param itemId
	 *            The Item to mark as read
	 * @param feedId
	 ...
	 *             If the user is not authenticated
	 */
	private void markItemAs(String itemId, String feedId, String markParameter,
			String markValue) throws GoogleReaderException {
		...
		parameters.add(new Parameter(markParameter, "user/" + userId
				+ markValue));
		String result = httpManager.post(Constants.URL_MARK_ITEM_AS_READ,
				parameters, true);
	...
	public void markLabelAsRead(String label) throws GoogleReaderException {
		...
		parameters.add(new Parameter("s", urlEncode("user/" + userId
				+ "/label/" + label)));
		parameters.add(new Parameter(Constants.PARAMETER_TOKEN, getToken()));
		String result = httpManager.post(Constants.URL_MARK_ALL_AS_READ,
				parameters, true);
	 ...
	 *             is empty
	 */
	public void addSubscription(FeedDescriptor feedDescriptor, String title)
			throws GoogleReaderException {
		if (feedDescriptor == null) {
			throw new IllegalArgumentException(
					"The feed descriptor cannot be null!");
		}
	 ...
	 *             If the feed is null
	 */
	public void removeSubscription(FeedDescriptor feedDescriptor)
			throws GoogleReaderException {
		if (feedDescriptor == null) {
			throw new IllegalArgumentException(
					"The feed descriptor cannot be null!");
	 ...
	 *             If the user is not authenticated
	 */
	public void addSubscription(String feedUrl, String title)
			throws GoogleReaderException {
	 ...
	 */
	private void editSubscription(String feedUrl, String title, boolean add)
			throws GoogleReaderException {
		...
		if (add) {
			parameters.add(new Parameter(Constants.EDIT_SUBSCRIPTION_ACTION,
					Constants.EDIT_SUBSCRIPTION_SUBSCRIBE));
		} else {
			parameters.add(new Parameter(Constants.EDIT_SUBSCRIPTION_ACTION,
					Constants.EDIT_SUBSCRIPTION_UNSUBSCRIBE));
		}
		parameters.add(new Parameter(Constants.EDIT_SUBSCRIPTION_FEED,
				urlEncode(feedUrl)));
		if (add) {
			parameters.add(new Parameter("t", urlEncode(title)));
		}
		parameters.add(new Parameter(Constants.PARAMETER_TOKEN, getToken()));
		String result = httpManager.post(Constants.URL_EDIT_SUSCRIPTION,
				parameters, true);
	 ...
	 *             the title is null or empty
	 */
	public void editSubscriptionLabel(String feedUrl, String title,
			String label, boolean add) throws GoogleReaderException {
		// LOG.trace ( "Editing a subscription (adding or removing one" );
		...
		if (add) {
			parameters.add(new Parameter(Constants.EDIT_SUBSCRIPTION_FEED_ADD,
					urlEncode("user/" + userId + "/label/" + label)));
		} else {
			parameters.add(new Parameter(
					Constants.EDIT_SUBSCRIPTION_FEED_REMOVE, urlEncode("user/"
							+ userId + "/label/" + label)));
		}
		parameters.add(new Parameter(Constants.EDIT_SUBSCRIPTION_FEED,
				urlEncode(feedUrl)));
		parameters.add(new Parameter(Constants.EDIT_SUBSCRIPTION_ACTION,
				Constants.EDIT_SUBSCRIPTION_EDIT_ACTION));
		if (add) {
			parameters.add(new Parameter("t", urlEncode(title)));
		}
		parameters.add(new Parameter(Constants.PARAMETER_TOKEN, getToken()));
